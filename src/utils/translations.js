export const translations = {
  en: {
    greeting: "Hey! I'm",
    name: "Erwan MENAGER",
    nickname: "But online I'm",
    nicknameValue: "Eowin",
    description: "I'm a full-stack developer and a calisthenics athlete.",
    projects: "→ See my projects",
    about: "→ More about me",
    contact: "→ Contact me",
    language: "FR",
    aboutDescription: "Hey! My name is Erwan MENAGER and I'm a french full-stack developer. I'm also a big fan of calisthenics and I've been practicing it since 2023. On average, I train 5 to 6 times a week. I plan to start competing when I feel my level is sufficient. (Yes, I'm a little demanding of myself).",
    
    // About section
    aboutTitle: "ABOUT",
    resumeButton: "↓ Resume",
    
    // Contact section
    contactTitle: "CONTACT",
    contactDescription: "Feel free to contact me to discuss projects, job opportunities or even a quick question.",
    nameLabel: "Name",
    emailLabel: "Email",
    messageLabel: "Message",
    sendButton: "Send Message",
    messageSent: "Message sent!",
    sending: "Sending...",
    emailError: "Failed to send message. Please try again.",
    cooldownMessage: "Please wait {time} before sending another message.",
    
    // Navigation
    homeLink: "Home",
    aboutLink: "About",
    contactLink: "Contact"
  },
  fr: {
    greeting: "Salut ! Je suis",
    name: "Erwan MENAGER",
    nickname: "Mais en ligne je suis",
    nicknameValue: "Eowin",
    description: "Je suis développeur full-stack et athlète de callisthénie.",
    projects: "→ Voir mes projets",
    about: "→ Plus sur moi",
    contact: "→ Me contacter",
    language: "EN",
    aboutDescription: "Salut ! Je m'appelle Erwan MENAGER et je suis développeur full-stack. Je suis également un grand passionné de callisthénie et j'en pratique depuis 2023. En moyenne, je m'entraîne 5 à 6 fois par semaine. Je compte me lancer dans les compétitions quand j'estimerai que mon niveau est suffisant. (Oui, je suis un peu exigeant avec moi-même).",
    
    // About section
    aboutTitle: "À PROPOS DE MOI",
    resumeButton: "↓ CV",
    
    // Contact section
    contactTitle: "CONTACT",
    contactDescription: "N'hésitez pas à me contacter pour discuter de projets, d'opportunités professionnelles ou même une simple question.",
    nameLabel: "Nom",
    emailLabel: "Email",
    messageLabel: "Message",
    sendButton: "Envoyer",
    messageSent: "Message envoyé !",
    sending: "Envoi en cours...",
    emailError: "Échec de l'envoi du message. Veuillez réessayer.",
    cooldownMessage: "Veuillez attendre {time} avant d'envoyer un autre message.",
    
    // Navigation
    homeLink: "Accueil",
    aboutLink: "À Propos",
    contactLink: "Contact"
  }
};